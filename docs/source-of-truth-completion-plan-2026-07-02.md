...
- `ROTA-001`: Shared backend rota output is deployed. The separate Android Driver App, not the Portal, is the intended driver experience; the Portal driver dashboard is internal diagnostic/staging only. See `docs/rota-001-driver-read-only-rota-2026-07-09.md`.
- `ROTA-002`: Deployed 2026-07-16. Added additive shift publish/status/audit migration, manager draft/publish/cancel UI, and driver filtering to published/updated own shifts. See `docs/rota-002-publish-status-audit-2026-07-09.md`.
- `EVENT-001-DESIGN`: Deployed 2026-07-16. Added additive event/thread/ack schema foundation with compatibility links to `messages`, company-scoped manager policies, driver assigned/broadcast visibility, and a security-invoker driver event view. The first runtime producer now writes a rota-publication event; driver UI/push/realtime behaviour remains pending. See `docs/event-001-operational-event-spine-design-2026-07-09.md`.
