...
export async function loadPlanningDomains<
  Core,
  Driver,
  Vehicle,
  Job,
...
>(loaders: {
  ...
  const settled = await Promise.allSettled([
    ...
    loaders.vehicles(),
    loaders.jobs(),
    loaders.recordedHours(),
  ]);
  const unwrap = <T>(
    result: PromiseSettledResult<DomainResult<T>>,
    fallback: T,
  ): DomainResult<T> =>
    result.status === "fulfilled"
      ? result.value
      : { data: fallback, error: toError(result.reason) };
  return {
...
export function buildBulkAssignmentPreview(input: {
  ...
  entitlements?: Readonly<Record<string, readonly LicenceEntitlement[]>>;
}): BulkCandidate[] {
  ...
  for (const slotId of input.slotIds) {
    ...
    for (const driverId of input.driverIds) {
      ...
      if (
        !entitlementCoversVehicleClass(
          input.entitlements?.[driverId] ?? [],
          slot.required_vehicle_class,
        )
      ) {
        status = "UNAVAILABLE";
        reason = "Licence entitlement does not cover this vehicle type";
      } else if (
        input.availability.some(
          (entry) =>
            entry.driver_id === driverId &&
            entry.starts_on <= slot.slot_date &&
            entry.ends_on >= slot.slot_date,
        )
      ) {
        status = "UNAVAILABLE";
        reason = "Holiday or other absence";
      } else if (
        workingAssignments.some(
          (entry) => entry.driver_id === driverId && entry.slot_id === slot.id,
        )
      ) {
        status = "CONFLICT";
        ...
        status = "UNAVAILABLE";
        reason = "No vacancy remains";
      } else if (
        workingAssignments.some((entry) => {
          if (entry.driver_id !== driverId) return false;
          const occupied = input.slots.find(
            (candidate) => candidate.id === entry.slot_id,
          );
          return occupied ? dutiesOverlap(occupied, slot) : false;
        })
      ) {
        status = "CONFLICT";
        reason = "Overlaps another duty";
      } else if ((input.regimes[driverId] ?? "unknown") === "unknown") {
        status = "NEEDS_REVIEW";
        reason = "Rules need confirming";
      }
      output.push({
        driverId,
        slotId: slot.id,
...
function toError(value: unknown): Error {
  return value instanceof Error
    ? value
    : new Error("Planning data could not be loaded");
}
