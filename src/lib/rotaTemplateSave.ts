...
export type RotaTemplateRequirementDraft = {
  ...
  endTime: string;
  headcount: number;
  vehicleClass: PlanningVehicleClass | "";
};
...
type SubmitDependencies = {
  rpc: (
    ...
    signal: AbortSignal,
  ) => PromiseLike<RotaTemplateRpcResponse>;
...
export function buildRotaTemplateCreateArgs(input: {
  ...
  const slots = input.requirements.map((entry, sortOrder) => {
    ...
    if (
      !Number.isInteger(entry.cycleDay) ||
      entry.cycleDay < 1 ||
      entry.cycleDay > input.cycleLength ||
      !roleLabel ||
      !validTime ||
      entry.startTime === entry.endTime ||
      !Number.isInteger(entry.headcount) ||
      entry.headcount < 1 ||
      entry.headcount > 50
    ) {
      throw new Error(
        "Check each requirement has a day, role, different start and finish times, and 1 to 50 people.",
      );
    }
    return {
      ...
      required_vehicle_class: entry.vehicleClass || null,
  ...
  try {
    ...
    if (response.error) {
      ...
      return {
        status: "server_error",
        message:
          "We couldn't save this staffing pattern. Nothing was saved. Check the entries and try again.",
      };
    }
    ...
    if (!outcome) {
      dependencies.reportTechnicalError?.(
        new Error("Template RPC returned no usable template identifier"),
      );
      return {
        status: "uncertain",
        message:
    ...
    const visibleAfterRefresh = await dependencies.refreshAndSelect(
      outcome.templateId,
    );
    if (!visibleAfterRefresh) {
      return {
        ...
        message:
          "The pattern was saved, but the refreshed pattern list did not include it. Refresh before trying again.",
      };
    }
    ...
    return {
      status: "uncertain",
      message:
        timedOut || error instanceof RotaTemplateTimeoutError
          ? "We couldn't confirm whether this pattern was saved. Refresh the pattern list before trying again."
...
function readCreateOutcome(
  data: Json | null,
): { templateId: string; replayed: boolean } | null {
