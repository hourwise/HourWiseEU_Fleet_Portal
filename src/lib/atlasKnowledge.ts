...
const GLOSSARY: ReadonlyArray<readonly [string, string]> = [
  ['proof of delivery', 'pod'], ['delivery note', 'pod'], ['delivery notes', 'pod'], ['lorry', 'vehicle'], ['lorries', 'vehicle'], ['unit', 'vehicle'], ['off road', 'vor'],
...
export function resolveAtlasQuestion(question: string, snapshot?: AtlasQuerySnapshot): AtlasKnowledgeResolution {
  ...
  const legacyIntent = /what needs attention/.test(normalizedQuestion) && !/and tomorrow|and yesterday/.test(normalizedQuestion) ? 'morning_briefing' : toLegacyIntent(canonicalIntents[0] ?? 'unknown');
  const faq = canonicalIntents.length === 1 ? FAQS[canonicalIntents[0]] : undefined;
  const confidence = scoreConfidence(normalizedQuestion, canonicalIntents, entities, Boolean(faq));
  const clarification = canonicalIntents.length > 1 && hasAmbiguousCompound(normalizedQuestion)
  ...
  return { canonicalIntents, legacyIntent, normalizedQuestion, entities, temporalPhrases, confidence, confidenceBand: confidence >= 0.8 ? 'high' : confidence >= 0.55 ? 'medium' : 'low', clarification, faqAnswer: faq?.answer ?? null, navigationTarget: faq?.href ?? null, negatedTerms };
}
...
function classifyCanonicalIntents(question: string, temporal: AtlasTemporalPhrase[], negated: string[]): AtlasCanonicalIntent[] {
  ...
  add('asset_readiness_reason', /why .* (blocked|vor)|readiness reason/);
  ...
  add('asset_compliance', /(vehicle|asset).*compliance|compliance.*(vehicle|asset)/);
  ...
  add('jobs_acknowledgement', /unacknowledged|awaiting acknowledgement|not acknowledged/);
  add('jobs_exceptions', /delayed|exception job|vehicle issue|site issue|route issue|unable_to_complete/);
  ...
  if (negated.length > 0 && intents.some((intent) => ['assets_prohibited', 'asset_defects', 'driver_compliance'].includes(intent))) return ['unknown'];
  return intents.length > 0 ? intents.slice(0, 2) : ['unknown'];
}
