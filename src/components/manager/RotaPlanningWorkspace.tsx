...
import {
  assessCandidate, availabilityForDate, availabilityLabel, buildCoverageRows, formatDuration,
...
type RotaTemplate = { id: string; name: string; description: string | null; cycle_length_days: number };
type TemplateSlot = { id: string; template_id: string; cycle_day: number; role_label: string; start_time: string; end_time: string; required_headcount: number; required_skill: string | null; sort_order: number };
type PlannedRun = { id: string; rota_slot_id: string | null; rota_slot_assignment_id: string | null; run_date: string; run_label: string; start_time: string; end_time: string; status: string };
...
export function RotaPlanningWorkspace({ onOpenJobPlanner }: { onOpenJobPlanner?: (shiftId: string) => void }) {
  ...
  const toDate = useMemo(() => format(addDays(parseISO(fromDate), 6), 'yyyy-MM-dd'), [fromDate]);
  const dates = useMemo(() => planningDates(fromDate, toDate), [fromDate, toDate]);
  ...
  useEffect(() => { if (!selectedTemplateId && snapshot.templates[0]) setSelectedTemplateId(snapshot.templates[0].id); }, [selectedTemplateId, snapshot.templates]);

  const coverageRows = useMemo(() => buildCoverageRows(snapshot.slots, snapshot.assignments, dates), [dates, snapshot.assignments, snapshot.slots]);
  const selectedSlot = snapshot.slots.find((slot) => slot.id === selectedSlotId) ?? null;
  const trailers = useMemo(() => vehicles.filter((vehicle) => vehicle.vehicle_class === 'trailer' || vehicle.vehicle_type === 'trailer'), [vehicles]);
  const poweredVehicles = useMemo(() => vehicles.filter((vehicle) => !trailers.some((trailer) => trailer.id === vehicle.id)), [trailers, vehicles]);
  const unallocatedJobs = useMemo(() => { const allocated = new Set(snapshot.run_jobs.filter((relation) => relation.status !== 'cancelled').map((relation) => relation.job_id)); return jobs.filter((job) => !allocated.has(job.id)); }, [jobs, snapshot.run_jobs]);
  const review = useMemo(() => buildReview(snapshot), [snapshot]);
  const moveWeek = (amount: number) => setFromDate(format(addDays(parseISO(fromDate), amount * 7), 'yyyy-MM-dd'));
  const today = () => setFromDate(format(startOfWeek(new Date(), { weekStartsOn: 1 }), 'yyyy-MM-dd'));
...
function CoverageView({ rows, dates, loading, availability, onSelectSlot }: { rows: ReturnType<typeof buildCoverageRows>; dates: string[]; loading: boolean; availability: PlanningAvailability[]; onSelectSlot: (id: string) => void }) {
  const gaps = rows.reduce((total, row) => total + dates.reduce((sum, date) => sum + row.cells[date].vacancies, 0), 0);
  return <section className="overflow-hidden rounded-2xl border border-brand-border bg-brand-card"><div className="flex items-center justify-between border-b border-brand-border px-5 py-4"><div><h2 className="text-lg font-black text-white">Staffing coverage</h2><p className="text-xs text-slate-400">Select a filled/required count to inspect people and fill vacancies.</p></div><span className={`rounded-full px-3 py-1 text-xs font-black ${gaps ? 'bg-amber-500/15 text-amber-200' : 'bg-emerald-500/15 text-emerald-200'}`}>{gaps ? `${gaps} vacancies` : 'Fully covered'}</span></div><div className="max-h-[calc(100vh-275px)] overflow-auto"><table className="w-full min-w-[980px] border-collapse"><thead className="sticky top-0 z-20 bg-brand-dark"><tr><th className="sticky left-0 z-30 min-w-[190px] border-b border-r border-brand-border bg-brand-dark p-3 text-left text-[10px] font-black uppercase tracking-widest text-slate-500">Requirement</th>{dates.map((date) => <th key={date} className="min-w-[125px] border-b border-brand-border p-3 text-center"><span className="block text-[10px] font-black uppercase text-slate-500">{format(parseISO(date), 'EEE')}</span><span className="text-sm font-black text-white">{format(parseISO(date), 'd MMM')}</span></th>)}</tr></thead><tbody className="divide-y divide-brand-border">{loading ? <tr><td colSpan={8} className="p-12 text-center text-sm text-slate-400">Loading coverage…</td></tr> : rows.length === 0 ? <tr><td colSpan={8} className="p-12 text-center text-sm text-slate-400">No staffing demand yet. Choose a pattern and apply it to this week.</td></tr> : rows.map((row) => <tr key={row.key}><th className="sticky left-0 z-10 border-r border-brand-border bg-brand-card p-3 text-left"><span className="block text-sm font-bold text-white">{row.roleLabel}</span><span className="text-[10px] text-slate-500">{row.startTime.slice(0, 5)}–{row.endTime.slice(0, 5)}</span></th>{dates.map((date) => { const cell = row.cells[date]; const leaveCount = availability.filter((entry) => entry.starts_on <= date && entry.ends_on >= date).length; return <td key={date} className="p-2 text-center">{cell.slot ? <button type="button" onClick={() => onSelectSlot(cell.slot!.id)} className={`w-full rounded-xl border px-3 py-3 text-left transition hover:-translate-y-0.5 ${cell.vacancies ? 'border-amber-500/30 bg-amber-500/10' : 'border-emerald-500/20 bg-emerald-500/5'}`}><span className="block text-lg font-black text-white">{cell.filled}/{cell.required}</span><span className={`text-[10px] font-bold ${cell.vacancies ? 'text-amber-200' : 'text-emerald-200'}`}>{cell.vacancies ? `${cell.vacancies} open` : 'Covered'}</span>{leaveCount ? <span className="mt-1 block text-[9px] text-slate-500">{leaveCount} away</span> : null}</button> : <span className="text-xs text-slate-700">—</span>}</td>; })}</tr>)}</tbody></table></div></section>;
}

function PeopleView({ drivers, dates, slots, assignments, availability, onSelectDriver }: { drivers: Driver[]; dates: string[]; slots: PlanningSlot[]; assignments: PlanningAssignment[]; availability: PlanningAvailability[]; onSelectDriver: (id: string) => void }) {
  return <section className="overflow-hidden rounded-2xl border border-brand-border bg-brand-card"><div className="border-b border-brand-border px-5 py-4"><h2 className="text-lg font-black text-white">People at a glance</h2><p className="text-xs text-slate-400">Duty, rest, leave and conflicts across the selected week.</p></div><div className="max-h-[calc(100vh-275px)] overflow-auto"><table className="w-full min-w-[980px]"><thead className="sticky top-0 z-20 bg-brand-dark"><tr><th className="sticky left-0 z-30 min-w-[200px] border-b border-r border-brand-border bg-brand-dark p-3 text-left text-[10px] font-black uppercase tracking-widest text-slate-500">Driver</th>{dates.map((date) => <th key={date} className="min-w-[120px] border-b border-brand-border p-3 text-center text-xs font-black text-white">{format(parseISO(date), 'EEE d')}</th>)}</tr></thead><tbody className="divide-y divide-brand-border">{drivers.map((driver) => <tr key={driver.id}><th className="sticky left-0 z-10 border-r border-brand-border bg-brand-card p-3 text-left"><button type="button" onClick={() => onSelectDriver(driver.id)} className="text-sm font-bold text-white hover:text-brand-accent">{driver.full_name ?? 'Unnamed driver'}</button></th>{dates.map((date) => { const away = availabilityForDate(availability, driver.id, date); const duties = assignments.filter((entry) => entry.driver_id === driver.id && slots.find((slot) => slot.id === entry.slot_id)?.slot_date === date).map((entry) => slots.find((slot) => slot.id === entry.slot_id)).filter((slot): slot is PlanningSlot => Boolean(slot)); const conflict = duties.length > 1; const label = away ? availabilityLabel(away.availability_type) : conflict ? 'Conflict' : duties[0]?.role_label ?? 'Rest'; return <td key={date} className="p-2"><button type="button" onClick={() => onSelectDriver(driver.id)} className={`w-full rounded-lg border px-2 py-3 text-xs font-black ${away ? 'border-violet-500/30 bg-violet-500/10 text-violet-200' : conflict ? 'border-red-500/30 bg-red-500/10 text-red-200' : duties[0] ? 'border-sky-500/30 bg-sky-500/10 text-sky-200' : 'border-white/5 bg-white/[0.02] text-slate-500'}`}>{label}<span className="mt-1 block text-[9px] font-medium opacity-75">{duties[0] ? `${duties[0].start_time.slice(0, 5)}–${duties[0].end_time.slice(0, 5)}` : away?.note ?? 'No duty'}</span></button></td>; })}</tr>)}</tbody></table></div></section>;
}
...
function LeaveView({ drivers, dates, availability, policies, onAdd }: { drivers: Driver[]; dates: string[]; availability: PlanningAvailability[]; policies: LeavePolicy[]; onAdd: (driverId: string) => void }) {
  return <section className="overflow-hidden rounded-2xl border border-brand-border bg-brand-card"><div className="flex items-center justify-between border-b border-brand-border px-5 py-4"><div><h2 className="text-lg font-black text-white">Leave & availability</h2><p className="text-xs text-slate-400">Select a driver to record a date range. Company capacity rules are planning policy.</p></div><span className="text-xs text-slate-500">{policies.length ? `${policies.length} capacity policies` : 'Capacity policy not configured'}</span></div><div className="max-h-[calc(100vh-275px)] overflow-auto"><table className="w-full min-w-[980px]"><thead className="sticky top-0 z-20 bg-brand-dark"><tr><th className="sticky left-0 z-30 min-w-[200px] border-b border-r border-brand-border bg-brand-dark p-3 text-left text-[10px] font-black uppercase text-slate-500">Driver</th>{dates.map((date) => <th key={date} className="border-b border-brand-border p-3 text-xs font-black text-white">{format(parseISO(date), 'EEE d')}</th>)}</tr></thead><tbody className="divide-y divide-brand-border">{drivers.map((driver) => <tr key={driver.id}><th className="sticky left-0 z-10 border-r border-brand-border bg-brand-card p-3 text-left"><button type="button" onClick={() => onAdd(driver.id)} className="text-sm font-bold text-white hover:text-brand-accent">{driver.full_name ?? 'Unnamed driver'} <Plus size={12} className="inline" /></button></th>{dates.map((date) => { const entry = availabilityForDate(availability, driver.id, date); return <td key={date} className="p-2 text-center"><button type="button" onClick={() => onAdd(driver.id)} className={`w-full rounded-lg border px-2 py-3 text-[10px] font-black ${entry ? 'border-violet-500/30 bg-violet-500/10 text-violet-200' : 'border-white/5 text-slate-700 hover:text-slate-400'}`}>{entry ? availabilityLabel(entry.availability_type) : 'Available'}</button></td>; })}</tr>)}</tbody></table></div></section>;
}
...
function DriverDrawer({ driver, slots, assignments, availability, workSessions, regime, onClose }: { driver: Driver | null; slots: PlanningSlot[]; assignments: PlanningAssignment[]; availability: PlanningAvailability[]; workSessions: WorkSession[]; regime: PlanningRegime; onClose: () => void }) {
  if (!driver) return null;
  const duties = assignments.filter((entry) => entry.driver_id === driver.id).map((entry) => slots.find((slot) => slot.id === entry.slot_id)).filter((slot): slot is PlanningSlot => Boolean(slot)).sort((a, b) => `${a.slot_date}${a.start_time}`.localeCompare(`${b.slot_date}${b.start_time}`));
  const actual = workSessions.filter((entry) => entry.user_id === driver.id).reduce((total, entry) => total + (entry.total_work_minutes ?? 0), 0);
  return <Drawer title={driver.full_name ?? 'Unnamed driver'} subtitle="Driver planning summary" onClose={onClose}><div className="grid grid-cols-2 gap-3"><Metric label="Recorded" value={formatDuration(actual)} /><Metric label="Planned duty" value={formatDuration(plannedMinutesForDriver(driver.id, slots, assignments))} /></div><div className={`mt-4 rounded-xl border p-3 ${regime === 'unknown' ? 'border-amber-500/30 bg-amber-500/10' : 'border-brand-border bg-white/[0.03]'}`}><p className="text-xs font-black text-white">{regimeLabel(regime)}</p><p className="mt-1 text-[10px] text-slate-400">Recorded, planned and unknown information remain separate. Duty time is not treated as driving time.</p></div><h3 className="mt-5 text-[10px] font-black uppercase text-slate-500">Assigned duties</h3><div className="mt-2 space-y-2">{duties.map((duty) => <div key={duty.id} className="rounded-lg border border-brand-border p-3"><p className="text-xs font-bold text-white">{format(parseISO(duty.slot_date), 'EEE d MMM')} · {duty.role_label}</p><p className="text-[10px] text-slate-500">{duty.start_time.slice(0, 5)}–{duty.end_time.slice(0, 5)}</p></div>)}</div><h3 className="mt-5 text-[10px] font-black uppercase text-slate-500">Upcoming leave</h3>{availability.filter((entry) => entry.driver_id === driver.id).map((entry) => <p key={entry.id} className="mt-2 rounded-lg bg-violet-500/10 p-3 text-xs text-violet-200">{availabilityLabel(entry.availability_type)} · {entry.starts_on}–{entry.ends_on}</p>)}</Drawer>;
}
...
function cycleDayLabel(day: number, length: number) { return length === 7 ? ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'][day - 1] ?? `Day ${day}` : `Day ${day}`; }
function regimeLabel(regime: PlanningRegime) { return regime === 'assimilated_aetr' ? 'Assimilated / AETR planning profile' : regime === 'gb_domestic' ? 'GB domestic planning profile' : regime === 'other_not_applicable' ? 'Other / not applicable' : 'Rules need confirming'; }
function buildReview(snapshot: Snapshot) { const active = snapshot.assignments.filter((entry) => entry.status !== 'cancelled'); return { duties: active.length, vacancies: snapshot.slots.reduce((total, slot) => total + Math.max(slot.required_headcount - active.filter((entry) => entry.slot_id === slot.id).length, 0), 0), missingVehicles: active.filter((entry) => !entry.vehicle_id).length, availabilityConflicts: active.filter((entry) => { const slot = snapshot.slots.find((item) => item.id === entry.slot_id); return Boolean(slot && availabilityForDate(snapshot.availability, entry.driver_id, slot.slot_date)); }).length, unstaffedRuns: snapshot.runs.filter((run) => !run.rota_slot_assignment_id).length, unallocatedRunJobs: snapshot.run_jobs.filter((entry) => !entry.projected_job_assignment_id).length }; }
