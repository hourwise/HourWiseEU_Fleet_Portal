-- Batch 17: typed source-link metadata and privacy-bounded timeline labels.
-- Navigation is represented as a small key. The browser resolves that key
-- through a typed registry and never trusts a URL returned by the database.
...
begin
  ...
  with fleet_rows as (
    select
      fe.*,
      case when fe.payload ->> 'job_assignment_id' ~* '^[0-9a-f-]{36}$' then (fe.payload ->> 'job_assignment_id')::uuid end as payload_assignment_id,
      case when fe.payload ->> 'proposal_id' ~* '^[0-9a-f-]{36}$' then (fe.payload ->> 'proposal_id')::uuid end as payload_proposal_id,
      case when fe.payload ->> 'vehicle_id' ~* '^[0-9a-f-]{36}$' then (fe.payload ->> 'vehicle_id')::uuid end as payload_vehicle_id,
      case when fe.payload ->> 'trailer_id' ~* '^[0-9a-f-]{36}$' then (fe.payload ->> 'trailer_id')::uuid end as payload_trailer_id
  ...
  source_rows as (
    select
      ...
      case
        when j.reference is not null then 'Job ' || left(j.reference, 120)
        when fe.related_shift_id is not null then 'Shift ' || left(fe.related_shift_id::text, 8)
        when coalesce(ja.id, fe.payload_assignment_id) is not null then 'Job assignment ' || left(coalesce(ja.id, fe.payload_assignment_id)::text, 8)
        else 'Fleet event ' || left(fe.id::text, 8)
      end as entity_label,
    ...
    left join public.jobs j on j.company_id = actor_company_id and j.id = ja.job_id
    ...
    select
      ...
      case
        when j.reference is not null then 'Job ' || left(j.reference, 120)
        when ap.target_entity_type = 'vehicle' and v.reg_number is not null then 'Vehicle ' || left(v.reg_number, 40)
        when ap.target_entity_type = 'driver' then 'Driver compliance record'
        else initcap(replace(ap.target_entity_type, '_', ' ')) || ' ' || left(ap.target_entity_id::text, 8)
      end as entity_label,
    ...
    left join public.jobs j on j.company_id = actor_company_id and j.id = ja.job_id
    left join public.vehicles v on v.company_id = actor_company_id and ap.target_entity_type = 'vehicle' and v.id = ap.target_entity_id
    ...
    select
      ...
      case when audit.resource_type = 'job_assignment' and j.reference is not null then 'Job ' || left(j.reference, 120) else initcap(replace(coalesce(audit.resource_type, 'audit event'), '_', ' ')) || coalesce(' ' || left(audit.resource_id::text, 8), '') end as entity_label,
    ...
    left join public.job_assignments audit_assignment on audit_assignment.company_id = actor_company_id and audit.resource_type = 'job_assignment' and audit_assignment.id = audit.resource_id
    left join public.jobs j on j.company_id = actor_company_id and j.id = audit_assignment.job_id
    ...
    select
      ...
      case when handling.source_type = 'job_assignment' and j.reference is not null then 'Job ' || left(j.reference, 120) else initcap(replace(handling.source_type, '_', ' ')) || ' ' || left(handling.source_id::text, 8) end as entity_label,
      ...
      case when handling.source_type = 'driver_compliance' then 'drivers' when handling.source_type in ('job_assignment', 'shift') then 'jobs' else 'vehicles' end as navigation_key,
      handling.status <> 'resolved' as unresolved
    from public.operational_task_handlings handling
    left join public.job_assignments ja on ja.company_id = actor_company_id and handling.source_type = 'job_assignment' and ja.id = handling.source_id
    left join public.jobs j on j.company_id = actor_company_id and j.id = ja.job_id
    ...
    select
      ...
      case when j.reference is not null then 'Job ' || left(j.reference, 120) else 'Job assignment ' || left(evidence.job_assignment_id::text, 8) end as entity_label,
  ...
  select jsonb_build_object(
    ...
    'navigationKey', source_rows.navigation_key,
    'relationships', jsonb_strip_nulls(jsonb_build_object(
      'jobAssignmentId', source_rows.job_assignment_id,
      'proposalId', source_rows.proposal_id,
      'eventId', source_rows.related_event_id,
      'driverId', source_rows.driver_id,
      'vehicleId', source_rows.vehicle_id,
      'trailerId', source_rows.trailer_id
    ))
  )
  from source_rows
...
is 'Bounded manager-only operational timeline projection. Entity labels are privacy-bounded; navigation uses typed keys and relationships are explicit validated source IDs.';
