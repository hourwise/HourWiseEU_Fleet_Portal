create table if not exists public.tachograph_reconciliation_items (
  ...
  tacho_label text not null,
  summary text not null,
  app_driving_mins integer not null default 0,
  tacho_driving_mins integer not null default 0,
  metadata jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now()
);

create index if not exists idx_tacho_reconciliation_import
  on public.tachograph_reconciliation_items(import_id, recon_date desc);

create index if not exists idx_tacho_reconciliation_driver
  on public.tachograph_reconciliation_items(company_id, driver_id, recon_date desc);

alter table public.driver_tacho_compliance_signals
  add column if not exists reconciliation_summary jsonb not null default '{"matchedDays":0,"tachoOnlyDays":0,"appOnlyDays":0,"mismatchDurationDays":0,"mismatchActivityDays":0,"uncertainDays":0,"totalIssues":0}'::jsonb;
  ...
  select jsonb_build_object(
    ...
    'reconciliation', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'tachoLabel', r.tacho_label,
        'summary', r.summary
      ) order by r.recon_date desc)
      from public.tachograph_reconciliation_items r
      cross join range_start rs
      where r.company_id = p_company_id
        and r.driver_id = p_driver_id
        and r.recon_date >= rs.value::date
    ), '[]'::jsonb),
    ...
    'driverComplianceSignals', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'reconciliationSummary', s.reconciliation_summary,
        'hasData', s.has_data
  ...
  select jsonb_build_object(
    ...
    'importRecord', (
      select jsonb_build_object(
        ...
        'technicalEventCount', (ir.metadata ->> 'technical_event_count')::integer,
        'discrepancyCount', (ir.metadata ->> 'discrepancy_count')::integer,
        'highSeverityCount', (ir.metadata ->> 'high_severity_count')::integer
      )
      from import_row ir
